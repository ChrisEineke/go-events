 ...
 *    bus.Topic("topic2")
 *
 *    if !bus.Topic("topic1").HasListeners() {
 *        t.Fail()
 *    }
 *    if bus.Topic("topic2").HasListeners() {
 *        t.Fail()
 *    }
 *}
 *
 ...
 *    }
 *
 *    if bus.Topic("topic").HasListeners() {
 *        t.Fail()
 *    }
 *}
 *
 ...
 *}
 *
 *func BenchmarkFireIntIntArg(b *testing.B) {
 *    bus := New()
 *    callback := func(_ int, _ int) {}
 *    topic := bus.Topic("topic")
 *    topic.On(callback)
 *    for b.Loop() {
 *        topic.Fire(b.N, b.N)
