...
func (e *E1[T1]) removeHandlerByFuncValue(v reflect.Value) (*handler1[T1], error) {
...
func (e *E1[T1]) On(fn Func1[T1], options ...SubscriptionModifier) error {
	e.lock.Lock()
	defer e.lock.Unlock()

	handler, err := newHandler1(e, fn, options...)
	if err != nil {
		return err
	}
...
func (e *E1[T1]) Off(fn Func1[T1]) error {
	e.lock.Lock()
	defer e.lock.Unlock()
